...
use_tactile_sensor = args.tactile_sensor

# Make the robot env
env = RobotEnv(enable_tactile=use_tactile_sensor)

if args.left_controller:
    controller = VRPolicy(right_controller=False)
    # Make the data collector
    data_collector = DataCollecter(env=env, controller=controller, tactile_sensor=use_tactile_sensor)
    # Make the GUI
